...
export const definition = {
  ...
  description:
    "Start a continuous or interval sleep task loop with model selection and full tool access (browser, MCP, etc.)",
  inputSchema: {
    type: "object",
    properties: {
      goal: {
      ...
      model: {
        type: "string",
        enum: ["auto", "composer-1", "grok"],
        description: "Model to use: 'auto', 'composer-1' (default), or 'grok'",
      },
      mode: {
        type: "string",
      ...
      reuse_core_prompt: {
        type: "boolean",
        description: "If true and CORE_PROMPT.md exists in output_dir, skip generation and reuse existing prompt (default: false)",
      },
      enable_browser: {
        type: "boolean",
...
export async function handler(args) {
  ...
  try {
    ...
    const reuseCorePrompt = args.reuse_core_prompt === true;
    const enableBrowser = args.enable_browser !== false; // Default true
    const enableMcps = args.enable_mcps !== false; // Default true
    ...
    const selectedModel = resolveModel(modelInput);
    ...
    let corePrompt;
    let skippedGeneration = false;

    // Check if we can reuse existing CORE_PROMPT.md
    if (reuseCorePrompt && fs.existsSync(corePromptPath)) {
      corePrompt = fs.readFileSync(corePromptPath, "utf-8");
      skippedGeneration = true;
      logInfo(taskId, "Reusing existing CORE_PROMPT.md, skipping generation", {
        corePromptPath,
      });
    } else {
      // Run generation to create CORE_PROMPT.md
      const generationPrompt = buildGenerationPrompt({
        goal,
        inputMaterials: resolvedInputs,
        outputDir: resolvedOutputDir,
      });
      logInfo(taskId, "Generation started", { model: selectedModel });
      ...
      corePrompt = fs.existsSync(corePromptPath)
    ...
    const summary = [
      ...
      `Browser tools: ${enableBrowser ? "enabled" : "disabled"}`,
      `MCP tools: ${enableMcps ? "enabled" : "disabled"}`,
      skippedGeneration ? "Reused existing CORE_PROMPT.md (skipped generation)" : "Generated new CORE_PROMPT.md",
      `Goal: ${goal}`,
